...
static bool	is_redshift = false;

/*credentials for redshift*/
static const char* redshift_host=NULL;
static const char* redshift_dbname=NULL;
static const char* redshift_port=NULL;
static const char* redshift_username=NULL;
static const char* redshift_tempfolder=NULL;
...
main(int argc, char **argv)
{
	...
	static struct option long_options[] = {
		...
		{"compressEnc", required_argument, NULL, 'z'},
	...
	while ((c = getopt_long(argc, argv, "abcCd:E:f:F:h:ij:n:N:oOp:RsS:t:T:U:vwWxzZ:",
							long_options, &optindex)) != -1)
	{
		switch (c)
		{
			...
			case 'd':			/* database name */
				...
				redshift_dbname=pg_strdup(dbname);
				break;
			...
			case 'h':			/* server host */
				...
				redshift_host=pg_strdup(pghost);
				break;
			...
			case 'p':			/* server port */
				...
				redshift_port=pg_strdup(pgport);
				break;
			...
			case 'U':
				...
				redshift_username=pg_strdup(username);
				break;
			...
			case 'z': /* Store Compression Encoding from redshift in files */
				if(!is_redshift)
				   first_pass = true;
				redshift_tempfolder=pg_strdup(optarg);
				if(first_pass){
					if(redshift_tempfolder!=NULL){
				       char filepath[1000];
				       strcpy(filepath,"\0");
				       strcat(filepath,"rm -r ");
				       strcat(filepath,redshift_tempfolder);
				       strcat(filepath,"/*.*");
				     system(pg_strdup(filepath));
				    }
				   else exit_nicely(1);
				}
				break;
...
dumpTable(Archive *fout, TableInfo *tbinfo)
{
	if (tbinfo->dobj.dump && !dataOnly)
	{
		...
		if (tbinfo->relkind == RELKIND_SEQUENCE)
			...
			if(first_pass){/*analyze compression*/
				char queryRemote[1000];
				char filepath[1000];
		       strcpy(filepath,"\0");
		       strcat(filepath,redshift_tempfolder);
		       strcat(filepath,"/");
		       strcat(filepath,fmtId(tbinfo->dobj.name));
		       strcat(filepath,".txt");
		       strcpy(queryRemote,"\0");
		       strcat(queryRemote,"psql ");
		       strcat(queryRemote," --host=");
		       strcat(queryRemote,redshift_host);
		       strcat(queryRemote," --dbname=");
		       strcat(queryRemote,redshift_dbname);
		       strcat(queryRemote," --port=");
		       strcat(queryRemote,redshift_port);
		       strcat(queryRemote," --username=");
		       strcat(queryRemote,redshift_username);
		       strcat(queryRemote," -c 'analyze compression ");
		       strcat(queryRemote,fmtId(tbinfo->dobj.name));
		       strcat(queryRemote,"'> ");
		       strcat(queryRemote,filepath);
		       strcat(queryRemote," &");
		       //printf("%s",queryRemote);
		       system(strdup(queryRemote));
			  }
			else{
	...
	else
	{
		...
		if (tbinfo->relkind != RELKIND_MATVIEW)
		{
			...
			for (j = 0; j < tbinfo->numatts; j++)
			{
				...
				if(is_redshift){
					appendPQExpBuffer(q, " %s", getRedshiftCompression(fout,fmtId(tbinfo->dobj.name),tbinfo->attnames[j]));
				}
			...
			if(is_redshift){
				char* result= getRedshiftAttr(fout,fmtId(tbinfo->dobj.name));
				appendPQExpBufferStr(q, result);
				free(result);
			}
...
static char* getRedshiftAttr(Archive *fout,char *tableName){
		...
		int i=0;
		PQExpBuffer query = createPQExpBuffer();
		//dump distkey/distsytle
		appendPQExpBuffer(query, "select pg_table_def.column from pg_table_def where tablename = '%s' and distkey=true",
				tableName);
		...
		if (ntups == 1){
			char *attrName=pg_strdup(PQgetvalue(res, 0, 0));
			strcat(result,"distkey(");
			strcat(result,attrName);
			strcat(result,")");
		}
		else {
			PQclear(res);
			...
			resetPQExpBuffer(query);
				appendPQExpBuffer(query, "select reldiststyle from pg_class where relname = '%s'",
						tableName);
			res = ExecuteSqlQueryForSingleRow(fout, query->data);

			if(strcmp(pg_strdup(PQgetvalue(res, 0, 0)),"0") == 0){
				strcat(result,"diststyle ALL");
			}
			else if(strcmp(pg_strdup(PQgetvalue(res, 0, 0)),"8") == 0){
				strcat(result,"diststyle EVEN");
			}
		}
	...
	//dump sortkey
		appendPQExpBuffer(query, "select pg_table_def.column from pg_table_def where tablename='%s' and sortkey>0",
				tableName);

		res = ExecuteSqlQuery(fout, query->data, PGRES_TUPLES_OK);
		ntups = PQntuples(res);
		if(ntups>0) {
			strcat(result,"sortkey(");
		}

		for(i=0;i<ntups;i++){
			char *attrName=pg_strdup(PQgetvalue(res, i, 0));
			strcat(result,attrName);
			if(i!=(ntups-1)){
				strcat(result,",");
			}
...
static char* getRedshiftCompression(Archive *fout,char *tableName, char *attr){
           ...
           // get the encoding for given attribute
	       while(fscanf(fptr, "%s", &c) == 1) {
	    	   if(strcmp(attr,c)==0){
	    		 char tempencode[1000];
	  	         strcpy(tempencode,"\0");
	    		 fscanf(fptr, "%s %s", &c,&tempencode);
	    		 strcat(encoding," encode ");
	    		 strcat(encoding,tempencode);
	  	    	 break;
	    	   }
	       }
